...
/* *************
|   robot Functions
|   specfically for different actions for autonomous
*********** */
...
void moveArms(double angle) { //moves the arms to a certain angle
  ArmMotors.spinToPosition(angle, degrees);
}
void spinIntake (double rotations){
  FlapMotor1.spinFor(rotations, turns);
}
...
void redSquareLeftauton(void) {
  ...
  turnChassis(15,0); //turns the robot to face the cube to the right of the red square
  driveChassis(30,0); //drives the robot to the cube
  spinIntake (30); //runs the intake rollers for 30 rotations to intake the cube
  turnChassis(15,0); //turns the robot to face the cube in the other red square
  driveChassis(20,0); //drive the robot towards the cube 
  spinIntake (30); //runs the intake rollers for 30 rotations to intake the cube
  driveChassis (0,20); //drives the robot sideways to reach the goal
